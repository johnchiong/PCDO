...
const graceOptions = computed(() => {
	const maxGrace = props.coopProgram.program?.grace_period ?? 0
	return Array.from({ length: maxGrace + 1 }, (_, i) => i)
})
	...
	if (isOpen) {
		...
		if (firstUploaded?.upload) {
			selectedFile.value = {
				...
				url: `/admin/coopProgram/${props.coopProgram.id}/checklist/${firstUploaded.upload.id}/preview`
			}
		}
	} else {
...
function saveConsent() {
	...
	router.post(
		`/admin/coopProgram/${props.coopProgram.id}/consent`,
		{},
		{
			onSuccess: () => {
...
function openFilePreview(item: any) {
	selectedFile.value = {
		...
		url: `/admin/coopProgram/${props.coopProgram.id}/checklist/${item.upload.id}/preview`
	}
}
...
function handleUpload(index: number, item: ChecklistItem) {
	...
	forms[index].post(
		`/admin/coopProgram/${props.coopProgram.id}/checklist/upload`,
		{
			forceFormData: true,
			preserveScroll: true,
			onSuccess: () => {
...
function submitLoan() {
	...
	loanForm.post(
		`/admin/coopProgram/${props.coopProgram.id}/finalize-loan`,
		{
			onSuccess: () => {
			...
			onError: () => {
				toast.error('Failed to finalize loan. Please try again. Error: ' + JSON.stringify(loanForm.errors))
			}
		}
	)
}
...
function handleSaveProgress() {
	if (allUploadsDone.value) {
		...
		router.visit(`/admin/programs/${props.coopProgram.program?.id}`)
	}
}

function handleDelete(uploadId: number, fileName: string) {
	router.delete(
		`/admin/coopProgram/${props.coopProgram.id}/checklist/${uploadId}`,
		{
			onSuccess: () => {
...
<template>
	<AdminLayout :breadcrumbs="breadcrumbs" :key="props.coopProgram.cooperative.id">
		<div class="bg-gray-100/90 dark:bg-gray-900 min-h-screen">
					...
					<form v-if="canFinalizeLoan" @submit.prevent="submitLoan" class="space-y-4">
						...
						<div>
                           ...
                           text-gray-800 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-[#3b445c] px-3 py-1 rounded">
								...
								<button type="button"
									@click="loanForm.loan_amount = props.coopProgram.program?.max_amount || 0"
									class="bg-gray-100 dark:bg-[#334155] border border-gray-300 dark:border-[#475569]
                           text-gray-800 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-[#3b445c] px-3 py-1 rounded">
						...
						<div class="mt-4 border-t pt-4 border-gray-300 dark:border-gray-600">
							...
							<div class="flex gap-4 flex-col sm:flex-row">
								<label v-for="month in graceOptions" :key="month" class="flex-1 cursor-pointer">
									<div :class="[
										...
										<input type="radio" v-model="loanForm.with_grace" :value="month"
											class="hidden" />
				...
				<div v-for="(item, index) in filteredChecklist" :key="item.id"
					...
					<div v-if="item.upload" class="mb-3 flex items-center justify-between">
						<p class="text-sm text-gray-800 dark:text-gray-300">
							Uploaded File: <span class="truncate block max-w-[140px] md:max-w-[140px]"
								title="{{ file.file_name }}">
								<strong>{{ item.upload.file_name }}</strong>
							</span> </p>
						<div class="flex gap-4">
							<a :href="`/admin/coopProgram/${props.coopProgram.id}/checklist/${item.upload.id}/download`"
           ...
           dark:disabled:bg-[#0f172a] dark:disabled:text-gray-500 disabled:cursor-not-allowed"
							...
							:title="isDisabled(index) ? 'Upload previous document first' : ''" required />
						...
						<div class="flex flex-col md:flex-row overflow-hidden md:h-[75vh] max-h-[70vh] md:max-h-none">
							...
							<div class="flex-1 p-3 md:p-4 overflow-y-auto max-h-[60vh] md:max-h-[80vh]">
								<div v-if="selectedFile">
									<p
										class="text-gray-800 dark:text-gray-100 mb-3 text-sm md:text-base break-words whitespace-normal">
									...
									<div v-if="isMobile">
										<template v-if="selectedFile.name.toLowerCase().endsWith('.pdf')">
											<PdfViewer v-if="!pdfFailed" type="checklist" :url="selectedFile.url"
												:cooperative-id="props.coopProgram.cooperative.id"
												:program-id="props.coopProgram.program?.id" :file-id="selectedFile.id"
										...
										<template v-else
											-if="selectedFile.name.toLowerCase().match(/\.(jpg|jpeg|png|gif|bmp|webp)$/)">
