...
const selectedChecklists = ref<number[]>([]);
        ...
        <div class="bg-gray-100/90 dark:bg-gray-900 rounded-3xl min-h-screen px-4 py-6">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                <form @submit.prevent="handleSubmit" class="space-y-6">
                    ...
                    <div class="mt-4">
                        <h3 class="text-lg font-semibold mb-2 text-gray-700 dark:text-gray-300">Associated Checklists</h3>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-60 overflow-y-auto border p-4 rounded-lg bg-gray-50 dark:bg-gray-700">
                            <div v-for="item in checklist" :key="item.id" class="flex items-center justify-between">
                                ...
                                <div class="flex gap-2">
                                    <button v-if="editingChecklistId !== item.id" type="button" @click="startEdit(item)" class="text-blue-500 text-sm">Edit</button>
                                    <button v-if="editingChecklistId === item.id" type="button" @click="saveEdit(item.id)" class="text-green-500 text-sm">Save</button>
                                    <button v-if="editingChecklistId === item.id" type="button" @click="cancelEdit" class="text-gray-500 text-sm">Cancel</button>
                                    <button type="button" @click="deleteChecklist(item.id, item.name)" class="text-red-500 text-sm">Delete</button>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center justify-between opacity-50">
                                <div class="flex items-center gap-2">
                                    ...
                                    <input v-model="newChecklistName" placeholder="Add new checklist..." class="px-2 py-1 border rounded dark:bg-gray-600 w-full" />
                                </div>
                                <button type="button" @click="addChecklist" class="text-blue-500 text-sm">Add</button>
                            </div>
                        </div>
                    </div>
