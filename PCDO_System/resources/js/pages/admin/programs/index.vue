	...
	<AdminLayout :breadcrumbs="breadcrumbs">
		<div class="bg-gray-100/90 dark:bg-gray-900 rounded-3xl min-h-screen">
			<div class="px-5 md:px-8 pt-5 grid gap-6 md:grid-rows-[auto_1fr]">
				<div class="bg-gray-900/50 dark:bg-gray-800/50 rounded-3xl">
					<div class="px-5 md:px-8 pt-5">
						<div class="flex flex-col gap-6 p-6">
							...
							hover:shadow-2xl hover:-translate-y-1.5 transform transition-all block relative">
								...
								<button
									class="w-full sm:w-auto px-4 py-2 rounded-lg bg-green-600 hover:bg-green-700 text-white text-xl font-medium shadow transition"
									@click.prevent="goToAddProgram">
									+
								</button>
							</div>
						</div>
					...
					text-gray-800 dark:text-gray-100 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none">
											...
											<option v-for="program in props.programs" :key="program.id"
												:value="program.id">
												{{ program.name }}
											</option>
										</select>
					...
					text-gray-800 dark:text-gray-100 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" />
								...
								<div class="p-4 overflow-auto max-h-[80vh] bg-gray-50 dark:bg-gray-800 rounded-b-2xl">
									...
									<!-- Desktop PDF -->
									<iframe v-if="!isMobile" :src="`${pdfUrl}`" class="w-full h-[75vh] rounded"
										key="pdfUrl" @load="pdfLoading = false"></iframe>
				...
				<div class="bg-gray-900/50 dark:bg-gray-800/50 rounded-3xl p-5">
					...
					<div v-else class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
						<div v-for ="program in archivedPrograms" :key="program.id"
