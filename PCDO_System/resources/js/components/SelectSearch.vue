...
<template>
	<div class="relative w-full" v-click-outside="onOutside" ref="root">
		<input
			...
			@input="onInput"
			:disabled="disabled"
				class="pl-9 pr-3 w-full p-3 rounded-xl border border-gray-500 dark:border-gray-700
				bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100
				focus:outline-none focus:ring-2 focus:ring-indigo-500"
		/>
		<div v-if="open && filtered.length > 0" class="absolute w-full mt-1 border border-gray-300 dark:border-gray-600 bg-gray-100 dark:bg-gray-800 shadow-lg max-h-60 overflow-y-auto rounded-md z-10">
			<ul>
				<li v-for="item in filtered" :key="itemKey(item)" @click="selectItem(item)" class="px-4 py-2 cursor-pointer hover:bg-gray-300 dark:hover:bg-gray-700">
					{{ itemLabel(item) }}
				</li>
			</ul>
		</div>
