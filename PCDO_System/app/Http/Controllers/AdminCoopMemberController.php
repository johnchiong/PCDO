...
class AdminCoopMemberController extends Controller
{
    public function index(Cooperative $cooperative)
    {
        ...
        return Inertia::render('admin/cooperatives/members/index', [
            'cooperative' => $cooperative->only('id', 'name'),
            'members' => $members,
            'years' => $years,
            'breadcrumbs' => [
                ['title' => 'Cooperatives', 'href' => route('admin.cooperatives.index')],
                ['title' => $cooperative->name, 'href' => route('admin.cooperatives.show', $cooperative->id)],
                ['title' => 'Members', 'href' => route('admin.cooperatives.members.index', $cooperative->id)],
            ],
        ]);
    }
    ...
    {
        return Inertia::render('admin/cooperatives/members/create', [
            'cooperative' => $cooperative->only('id', 'name'),
            'breadcrumbs' => [
                ['title' => 'Cooperatives', 'href' => route('admin.cooperatives.index')],
                ['title' => $cooperative->name, 'href' => route('admin.cooperatives.show', $cooperative->id)],
                ['title' => 'Members', 'href' => route('admin.cooperatives.members.index', $cooperative->id)],
                ['title' => 'Add Member', 'href' => route('admin.cooperatives.members.create', $cooperative->id)],
            ],
        ]);
    }
    ...
    public function store(Request $request, Cooperative $cooperative)
    {
        ...
        return redirect()
            ->route('admin.cooperatives.members.index', $cooperative->id)
            ->with('success', 'Member created successfully.');
    }

    public function show(Cooperative $cooperative, CoopMember $member)
    {
        return Inertia::render('admin/cooperatives/members/show', [
            ...
            'breadcrumbs' => [
                ...
                ['title' => $cooperative->name, 'href' => route('admin.cooperatives.show', $cooperative->id)],
                ['title' => 'Members', 'href' => route('admin.cooperatives.members.index', $cooperative->id)],
                ['title' => $member->id.' - '.$member->first_name.' '.$member->last_name, 'href' => route('admin.cooperatives.members.show', [
    ...
    public function edit(Cooperative $cooperative, CoopMember $member)
    {
        ...
        return Inertia::render('admin/cooperatives/members/edit', [
            'cooperative' => $cooperative,
            'member' => $member->load('files'),
            'breadcrumbs' => [
                ['title' => 'Cooperatives', 'href' => route('admin.cooperatives.index')],
                ['title' => $cooperative->name, 'href' => route('admin.cooperatives.show', $cooperative->id)],
                ['title' => 'Members', 'href' => route('admin.cooperatives.members.index', $cooperative->id)],
                ['title' => $details, 'href' => route('admin.cooperatives.members.show', [$cooperative->id, $member->id])],
                ['title' => 'Edit', 'href' => route('admin.cooperatives.members.edit', [$cooperative->id, $member->id])],
            ],
        ]);
    }
    ...
    public function downloadFile(Cooperative $cooperative, CoopMember $member, $fileId)
    {
        ...
        if (! $file) {
            return redirect()
                ->route('admin.cooperatives.members.show', [$cooperative->id, $member->id])
